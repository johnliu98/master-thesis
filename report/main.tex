...
\begin{itemize}
    \item This research does not investigate the performance of various
        reinforcement learning controller. The goal is not to find the best
        controller. Instead, we aim at understanding and showcasing the
        predictive safety filter.
    \item The predictive safety filter is usually applied on a car racing
        problem. However, in this report, we will only consider path following
        problems. While the swing-up pendulum problem is used to showcase the
        predictive safety filter, it is not used to answer the research
        problem.
\end{itemize}
